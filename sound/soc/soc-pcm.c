...
static int soc_pcm_open(struct snd_pcm_substream *substream)
{
	...
	pr_err("asoc: %s <-> %s info:\n",
			codec_dai->name, cpu_dai->name);
	pr_err("asoc: rate mask 0x%x\n", runtime->hw.rates);
	pr_err("asoc: min ch %d max ch %d\n", runtime->hw.channels_min,
		 runtime->hw.channels_max);
	pr_err("asoc: min rate %d max rate %d\n", runtime->hw.rate_min,
		 runtime->hw.rate_max);
...
static void close_delayed_work(struct work_struct *work)
{
	...
	pr_err("pop wq checking: %s status: %s waiting: %s\n",
...
static int soc_dpcm_be_dai_startup(struct snd_soc_pcm_runtime *fe, int stream)
{
	...
	list_for_each_entry(dpcm_params, &fe->dpcm[stream].be_clients, list_be) {
		...
		pr_err("%s be->dai_link->name stream to be started %s\n",__func__,be->dai_link->name);
...
static int soc_dpcm_fe_dai_startup(struct snd_pcm_substream *fe_substream)
{
	...
	pr_err("%s start front end dai %s\n",__func__,fe->dai_link->name);
...
static int soc_dpcm_be_dai_hw_free(struct snd_soc_pcm_runtime *fe, int stream)
{
	...
	list_for_each_entry(dpcm_params, &fe->dpcm[stream].be_clients, list_be) {
		...
		struct snd_pcm_substream *be_substream =
			snd_soc_dpcm_get_substream(be, stream);
		pr_err("%s dailink name %s stream val %d\n", __func__, be->dai_link->name, stream);
		/* is this op for this BE ? */
		if (!snd_soc_dpcm_be_can_update(fe, be, stream)) {
			continue;
		}
