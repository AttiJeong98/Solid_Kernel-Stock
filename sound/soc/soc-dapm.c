			 ...
			 struct list_head *list, int event, bool power_up)
{
	...
	list_for_each_entry_safe(w, n, list, power_list) {
		ret = 0;
		if (power_up)
			pr_err(" %s Power up widgets w->name %s",__func__,w->name);
		else
			pr_err(" %s Power down widgets w->name %s",__func__,w->name);
				 ...
				 int mux, struct soc_enum *e)
{
	struct snd_soc_dapm_path *path;
	int found = 0;
	pr_err("%s widget name %s change %d\n",__func__,widget->name,change);
				   ...
				   struct snd_kcontrol *kcontrol, int connect)
{
	...
	if (widget->id != snd_soc_dapm_mixer &&
		...
		return -ENODEV;
	 pr_err("%s widget name %s connect %d\n",__func__,widget->name,connect);
	/* find dapm widget path assoc with kcontrol */
	list_for_each_entry(path, &widget->dapm->card->paths, list) {
		if (path->kcontrol != kcontrol)
			continue;
	...
	struct snd_ctl_elem_value *ucontrol)
{
	...
	pr_err("%s widget->name %s change %d val %d connect %d widget->value %d\n",
	...
	pr_err("%s widget->name %s new widget->value %d",__func__,widget->name,widget->value);
